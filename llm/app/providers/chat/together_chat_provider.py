...
class TogetherChatProvider:
    ...
    def generate(self, messages: list[dict[str, Any]], options: dict[str, Any]) -> ChatResult:
        ...
        model = options.get("together_default_chat_model") or options.get("chat_model_name")
        if not model:
            raise ValueError("Modelo de chat da Together não configurado")
